...
After having searched all light curves for flares~(see Section~\ref{sec:flarecatalog}), we calculate the deviation from a uniform \textbf{underlying} flare rate distribution with orbital, rotational, and synodic phase each using a customized A-D test in six steps:
\begin{enumerate}
\item Calculate $\phi$, the \textbf{expected frequency distrbution of flares} per phase \textbf{(expected distribution for short)}, using all valid data points in the de-trended light curves, taking into account the different observing cadences, and detection thresholds.
\item Sample $N$ flare occurrence phases $x_i$ with $i \in [1,N]$ from the expected distribution $\phi$ to get a distribution of flare phases, where $N$ is the total number of flares in the data. This step generates a list of flares that are uniformly distributed across all phases, modulated only by the phase coverage of the observations, and the detection thresholds of the light curves. \textbf{This list is a sample from the expected distribution we test against~(see Fig.~\ref{fig:cumdist}).} 
\item Repeat the previous step to obtain a large number of \textbf{samples}, and calculate the A-D statistic $A^2$ for each of them using the cumulative form $\hat{\phi}(x_i)$ of the \textbf{expected} distribution, where the $x_i$ are sorted in ascending order. 
...
\item Sort all $x_{i,obs}$ by phase in ascending order using the same start phase as the \textbf{expected} distribution.
...
\textbf{We also applied the above method to test for active longitudes excited by the close 4:7 $P_{rot}$:$P_{orb}$ commensurability~\citep{szabo2021changing,cale2021diving} of AU Mic and AU Mic b. The excitation of resonant oscillations in the stellar magnetic field of close-in star-planet systems was predicted by~\citet{lanza2022model}. We found no significant deviation from uniformity with the resonance period $P_{rot}/4$, either. However, for the AU Mic system specifically,~\citet{lanza2022model} estimate that this excitation will not occur due to the planet's relatively large distance from its host, consistent with our result.}
...
Third, flaring activity could be elevated through flaring SPI, but manifest as excess flares across all longitudes. \textbf{While the mechanism of flaring SPI is not well understood, we can imagine circumstances that create a large or spread-out SPI-induced flaring region, which would always be in view. Flaring region sizes increase with flare energy for intrinsic flares~\citep{sammis2000dependence, notsu2019kepler, howard2020evryflare}. It is therefore conceivable that extreme amounts of released energy during SPI might manifest in large flaring regions. Another scenario is efficient transport of the released energy or the magnetic field perturbation within the stellar magnetosphere before erupting in a flare. This could result in a broad range of longitudes at which SPI flares occur. Alternatively, the magnetic field lines connecting planet and star could be arranged such that the locations they map to on the stellar surface are distributed in a broad longitudinal range.}
...
\textbf{The authors would like to thank Gyula M. Szab\'o for comments that improved the quality of this work. }
