...
\begin{abstract}
Planets that move in close orbits around magnetically active stars can interact with their magnetic fields in a way that modulates the star's activity. This modulation in phase with the planet's orbit, such as enhanced X-ray activity, chromospheric spots, radio emission, or flares, are considered the clearest signs of magnetic star-planet interaction (SPI). However, the magnitude of this interaction is poorly constrained, and the intermittent nature of the interaction is a challenge for observers. 

AU Mic is the most actively flaring star hosting a planet known to date. A warm Neptune orbits AU Mic a 8.6 day orbit, makin it a promising target for magnetic SPI observations. In this study, we used optical light curves of this young, active M1 star obtained by the Transiting Exoplanet Survey Satellite to search for signs of flaring SPI with its innermost companion, AU Mic b. In the about 50 days of observations the distribution of flares with orbital, rotational and synodic phase was consistent with intrinsic stellar flaring. We discuss possible physical and instrumental reasons for the absence of SPI signal in AU Mic, and conclude that XXXX.
...
\item Calculate observing time per phase using all valid data points in each of the de-trended light curves, taking into account the different observing cadences, and multiplying with $F_k/F$ where $F_k$ is the number of flares detected in the light curve. The latter step accounts for the different detection thresholds of each light curve.
...
\item Repeat the previous step to obtain a large number of phase coverage distributions, and calculate the A-D statistic for all of them. In this study, we generated 10 000 sample distributions for each test.
...
The observed absence of flaring SPI can be explained in two ways. There can be observational biases that impede detection, and physical reasons that prevent flaring SPI to occur in the first place. We consider two physical explanations first.
