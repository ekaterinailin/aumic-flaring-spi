...
Since flaring is a statistical phenomenon, where individual flares are very difficult to predict even on the Sun~\citep{barnes2016}, and SPI flares are also expected to vary orbit to orbit in number and intensity~\citep{shkolnik2008,strugarek2015}, it is challenging to identify flares as triggered by SPI as compared to intrinsically. Perspectively, a difference in the flare frequency distribution (Fig.~\ref{fig:ffd}), which would arise as a kink in the power law distribution once a larger sample of flares accumulates and small differences in the distributions can be disentangled from detection efficiency effects and statistical uncertainty.
